...
void Estimator::processIMU(double dt, const Vector3d &linear_acceleration, const Vector3d &angular_velocity)
{
    ...
    // pre_integrations 里面存放着(WINDOW_SIZE + 1)个指针，指针指向的类型是IntegrationBase
    ...
    if (frame_count != 0)
    {
        ...
        tmp_pre_integration->push_back(dt, linear_acceleration, angular_velocity); // 这里放微元的后几个

        dt_buf[frame_count].push_back(dt); // 这里放微元的dt
...
bool Estimator::initialStructure()
{
    ...
    vector<SFMFeature> sfm_f;  // 存储的是每个特征对应的所有的被观测数据
