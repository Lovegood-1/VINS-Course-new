...
/**
 * 检测imu数据是否乱序，并放进自定义的类IMU_MSG中，并唤醒条件变量去获取数据
 * 
*/
    ...
    while (bStart_backend)
    {
        ...
        for (auto &measurement : measurements)
        {
            ...
            for (auto &imu_msg : measurement.first)
            {
                ...
                double img_t = img_msg->header + estimator.td;// 本数据对最晚时间  
                if (t <= img_t)
                {
                    if (current_time < 0)
                        current_time = t;
                    double dt = t - current_time; // IMU 距离图像的时间
                    assert(dt >= 0);
                    current_time = t;
                    ...
                    estimator.processIMU(dt, Vector3d(dx, dy, dz), Vector3d(rx, ry, rz)); // 传入微元
                    // printf("1 BackEnd imu: dt:%f a: %f %f %f w: %f %f %f\n",dt, dx, dy, dz, rx, ry, rz);
                }
                // 如果imu的时间戳t tt大于第k+1帧的图像时间戳img_t
